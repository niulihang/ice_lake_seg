...
for imagePath in glob.glob('../data/original/S2*_MSIL1C_***.SAFE/GRANULE/L1C_**/IMG_DATA/'):
    ...
    tile = str(band3Path)[18:44] + str(band3Path[55:62])
    ...
    # export ndwi image
    print('tiff export start')
    # with rasterio.open('../data/original/Lake_' + tile + '.png', 'w', driver='PNG',
    #                    width=band8.width, height=band8.height, count=1,
    #                    dtype=rasterio.uint8, nodata=0, transform=band8.transform,
    #                    crs=band8.crs) as dst:
    #     dst.write(lakeL_mask_cc.astype(rasterio.uint8))
    label_img_data = np.squeeze(lakeL_mask_cc)
    label_img_data = label_img_data.astype(np.uint8)
    label_img = Image.fromarray(label_img_data, 'P')
    label_img.putpalette(bin_colormap)
    label_img.save('../data/original/Lake_' + tile + '.png')
    label_img.close()
    print('tiff export done')

    # Convert Raster to polygon shapefiles
    print('shpfile export start')
    with rasterio.Env():
        with rasterio.open('../data/original/Lake_' + tile + '.png') as src:
            image = src.read()
